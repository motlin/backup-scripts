...
pub async fn run(args: Args, cfg: &CleanCypressConfig, dry_run: bool) -> Result<()> {
    let cache_dir = args
        .cache_dir
    ...
    let result = async move {
        ...
        let bar = Arc::new(CommandBar::new("clean-cypress", candidates.len() as u64));
    ...
    if let Some((summary, items)) = result {
        ui::print_tree(&format!("clean-cypress: {summary}"), &items);
    }
    Ok(())
}
